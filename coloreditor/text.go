...
func (e *textView) PaintText(gtx layout.Context, material op.CallOp) {
	e.PaintTextColored(gtx, func(int) op.CallOp { return material })
}

// PaintTextColored clips and paints the visible text glyph outlines, allowing
// each visual line to choose its own paint material.
func (e *textView) PaintTextColored(gtx layout.Context, lineMaterial func(line int) op.CallOp) {
	m := op.Record(gtx.Ops)
	...
	startLine := 0
	startGlyph := 0
	for lineIdx, line := range e.index.lines {
	...
	glyphOffset := startGlyph
	for lineIdx := startLine; lineIdx < len(e.index.lines); lineIdx++ {
		lineInfo := e.index.lines[lineIdx]
		if lineInfo.ascent.Ceil()+lineInfo.yOff > viewport.Max.Y {
			break
		}
		endGlyph := glyphOffset + lineInfo.glyphs
		if endGlyph > len(e.index.glyphs) {
			endGlyph = len(e.index.glyphs)
		}
		it.material = lineMaterial(lineIdx)
		line := glyphs[:0]
		for _, g := range e.index.glyphs[glyphOffset:endGlyph] {
			var ok bool
		...
		if len(line) > 0 {
			it.flushLine(gtx, e.shaper, line)
		}
		glyphOffset = endGlyph
	}
