...
type Editor struct {
	...
	// TabWidth controls how many spaces a tab character is expanded to on input.
	TabWidth int
	// LineColor returns the color used to paint a visual line of text.
	// When nil, the editor uses the shared text color for all lines.
	LineColor func(line int) color.NRGBA
...
func (e *Editor) paintText(gtx layout.Context, material op.CallOp) {
	e.initBuffer()
	if e.LineColor != nil {
		e.text.PaintTextColored(gtx, func(line int) op.CallOp {
			m := op.Record(gtx.Ops)
			paint.ColorOp{Color: e.LineColor(line)}.Add(gtx.Ops)
			return m.Stop()
		})
		return
	}
...
func (e *Editor) normalizeInsertedText(s string) string {
